...
class Server(object):
    ...
    def save_global_model(self):
        model_path = self.datapath+self.dataset+"/model/"
    ...
    def save_results(self):
        ...
        result_path = self.datapath+self.dataset+"/results/"
    ...
    def test_metrics(self):
        ...
        for c in self.clients:
            ct, ns, auroc, auprc = c.test_metrics()
            tot_correct.append(ct*1.0)
            tot_auroc.append(auroc*ns)
            tot_auprc.append(auprc*ns)
            num_samples.append(ns)

            print(f"Client {c.id} test accuracy: {ct / ns}")
            print(f"Client {c.id} AUROC: {auroc}")
            print(f"Client {c.id} AUPRC: {auprc}")

            mean_auc=mean_auc+auroc

        ids = [c.id for c in self.clients]
        print(f"Mean test AUC: {mean_auc/2}")
        return ids, num_samples, tot_correct, tot_auroc, tot_auprc
